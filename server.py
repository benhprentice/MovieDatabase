...
conn.commit()
cursor_setup.execute('CREATE TABLE IF NOT EXISTS movies(title text, year integer, cast text, genres text)')
conn.commit()
cursor_setup.execute('DROP TABLE IF EXISTS movieCast')
conn.commit()
cursor_setup.execute('CREATE TABLE IF NOT EXISTS movieCast(title text, castList text)')
conn.commit()
cursor_setup.execute('DROP TABLE IF EXISTS movieGenres')
conn.commit()
cursor_setup.execute('CREATE TABLE IF NOT EXISTS movieGenres(title text, genre text)')
conn.commit()
# cursor_setup.execute('DROP TABLE IF EXISTS viewedMovies')
# conn.commit()
cursor_setup.execute('CREATE TABLE IF NOT EXISTS viewedMovies(title text, genre text, user text)')
conn.commit()
# cursor_setup.execute('DROP TABLE IF EXISTS moviesSearched')
# conn.commit()
cursor_setup.execute('CREATE TABLE IF NOT EXISTS moviesSearched(keyword text, title text, user text)')
conn.commit()
cursor_setup.close()
...
def home():
    ...
    if 'loggedin' in session:
        ...
        if request.method == 'POST' and 'titleSearched' in request.form:
            ...
            print(Xgenres[0][0])
            for i in Xgenres:
                cursor.execute('INSERT INTO moviesSearched (keyword, title, user) VALUES (?, ?, ?)', (title, i[0], session['username'],))
            conn.commit()
        ...
        if request.method == 'POST' and 'moviezz' in request.form:
            ...
            print(Xmovies[0][0])
            for i in Xmovies:
                cursor.execute('INSERT INTO viewedMovies (title, genre, user) VALUES (?, ?, ?)', (moviezz, i[0], session['username'],))
            conn.commit()
...
def profile():
    ...
    if 'loggedin' in session:
        ...
        user = cursor.fetchone()
        cursor.execute('SELECT DISTINCT title FROM viewedMovies WHERE user = ?', (session['username'],))
